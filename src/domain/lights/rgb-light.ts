...
import { xyToLoxoneHsv } from '../../utils/color';

/** Full-color Loxone ColorPickerV2 (pickerType = "RGB") */
export class RgbLight extends LoxoneLight {
  ...
  needsStatePrefetch(data: HueEventData): boolean {
    // Color data is missing but the event only carries brightness/on — we need
    // the current color from Hue to produce a complete hsv() value.
    return !data.color?.xy && (data.dimming !== undefined || data.on !== undefined);
  }
}
